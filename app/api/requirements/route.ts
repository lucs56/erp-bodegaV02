...
import { bomItems, bomSubstitutes, products, stockItems } from "../../../db/schema";
...
export async function GET() {
  try {
    ...
    const [productRows, itemRows, substituteRows, stock] = await Promise.all([db.select().from(products).orderBy(asc(products.code)), db.select().from(bomItems), db.select().from(bomSubstitutes).orderBy(asc(bomSubstitutes.priority)),db.select().from(stockItems)]);
    ...
    const effective=buildEffectiveBoms(records,approvedBoms);const calculated=calculateRequirements(records,effective.boms); const shortages=calculated.requirements.map((item)=>{const available=stock.find((s)=>s.materialCode===item.materialCode)?.quantity??0;return {...item,available,shortage:Math.max(0,item.total-available)};}).filter((item)=>item.shortage>0); return Response.json({ source: { live: Boolean(live), fetchedAt: live?.fetchedAt }, ...calculated,...effective,stockItems:stock.length,shortages,purchases:shortages });
