...
type Item={materialCode?:string;materialName?:string;category?:string;quantity?:number;unit?:string};
...
export async function POST(request:Request){
  try{
    ...
    const values=payload.items.map(item=>({materialCode:item.materialCode?.trim()??"",materialName:item.materialName?.trim()??"",category:item.category?.trim()||"Otros",quantity:Number(item.quantity),unit:item.unit?.trim()||"unidad"}));
    ...
    return Response.json({ok:true,imported:values.length,saved,updatedAt:now});
