    ...
    const operation=(async()=>{
      ...
      try {
        ...
        const payload=await requestProgram(
          endpoint,
          mode==="stored"?5_000:35_000,
          force?1:0,
        );
        changed=applyPayload(payload)||changed;

        // Si este dispositivo no fue el líder, recibe D1 mientras otro request
        // consulta Google. En ese caso hacemos únicamente lecturas de D1 hasta
        // que aparezca la versión nueva. Esto no genera llamadas adicionales a
        // Google y hace que todos los dispositivos vean el cambio en pocos
        // segundos, sin esperar al siguiente ciclo de 30 s.
        if(mode==="automatic"&&payload.source?.syncing){
          const firstFetchedAt=payload.source?.fetchedAt;
          for(let attempt=0;attempt<8;attempt+=1){
            await new Promise<void>((resolve)=>window.setTimeout(resolve,2_000));
            const storedPayload=await requestProgram("/api/program?stored=1",5_000,0);
            changed=applyPayload(storedPayload)||changed;
            const hasNewVersion=Boolean(
              storedPayload.source?.fetchedAt&&
              firstFetchedAt&&
              storedPayload.source.fetchedAt!==firstFetchedAt
            );
            if(hasNewVersion||!storedPayload.source?.syncing)break;
          }
        }
