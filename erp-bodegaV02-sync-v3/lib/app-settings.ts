...
export const DEFAULT_SETTINGS:OperationalSettings={spreadsheetId:"1XL44rx3sNKpxowAQzY1iSjy7s8lYOsPTMngD6xeBDPQ",syncIntervalSeconds:30,cacheSeconds:30,includedDepots:["2","13","C18","R18","2OB"]};
...
export async function readSettings():Promise<OperationalSettings>{
  try{
    ...
    // Migra el valor predeterminado anterior (30/15) sin tocar una
    // configuración elegida expresamente por el administrador.
    if(stored.syncIntervalSeconds===30&&stored.cacheSeconds===15)
      return{...stored,cacheSeconds:30};
    return stored;
