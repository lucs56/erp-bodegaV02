...
import { diffProgram, diffProgramDetailed } from "../lib/program-diff.ts";
...
test("describe los productos afectados para compartir el cambio",()=>{const result=diffProgramDetailed([base],[{...base,bottles:120},{...base,id:"c"}]);assert.equal(result.modifiedRecords[0].productCode,"P");assert.equal(result.addedRecords[0].bottles,100);});
